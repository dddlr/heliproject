...
static Mutex g_buffer_mutex;        // Creates a mutex for the buffer
...
ADCIntHandler(void)
{
	...
	// Locks the mutex to allow the buffer to write
	mutex_lock(g_buffer_mutex);
	// Place it in the circular buffer (advancing write index)
	writeCircBuf (&g_inBuffer, ulValue);
	// Unlocks the mutex after it finishes writing into the buffer
...
int main(void)
{
    ...
    // Waiting for the buffer to write
    mutex_wait(g_buffer_mutex);
